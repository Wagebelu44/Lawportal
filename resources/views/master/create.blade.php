					...
					<form method="post" action="{{url('/master')}}<?php if($__env->yieldContent('masterId')) { echo '/'.$__env->yieldContent('masterId');}?>" id="MasterForm" class="formular adminForm" autocomplete="off">
						...
						<div class="row">
							...
							<div class="col-sm-10 form-group">
								...
								<a href="{{ url('/user/create') }}?userrole_id=2" class="btn btn-primary pull-right">Add New Client</a>
							</div>
							<div class="clearfix"></div>
							...
							<div class="col-sm-10 form-group"><input type="file" name="file" class="filer_input_doc"></div>
							<div class="clearfix"></div>
							<div class="col-sm-10 col-sm-offset-2 form-group upload_google_drive_sec"></div>
							<div class="clearfix"></div>
							<div class="col-sm-10 form-group col-sm-offset-2">
...
<?php
$allowedEditorArr = array('case', 'todo', 'evaluation', 'incident', 'file_manager', 'judgement', 'contact');
?>
@if(in_array($data['master_type'], $allowedEditorArr))
...
@if($data['master_type'] == 'file_manager')
<script type="text/javascript">
CKEDITOR.replace( 'fileMatterEditor', {
	filebrowserUploadUrl: "{{route('ckeditor.upload', ['_token' => csrf_token() ])}}"
});
<?php if(isset($data['file_matter'])) : ?>
CKEDITOR.instances.fileMatterEditor.setData(`<?php echo $data['file_matter'];?>`);
<?php endif;?>
</script>
@endif
@if($data['master_type'] == 'judgement')
<script type="text/javascript">
CKEDITOR.replace( 'judgementEditor', {
	filebrowserUploadUrl: "{{route('ckeditor.upload', ['_token' => csrf_token() ])}}"
});
<?php if(isset($data['judgement_description'])) : ?>
CKEDITOR.instances.judgementEditor.setData(`<?php echo $data['judgement_description'];?>`);
<?php endif;?>
</script>
@endif
@if($data['master_type'] == 'contact')
<script type="text/javascript">
CKEDITOR.replace( 'contactEditor', {
	filebrowserUploadUrl: "{{route('ckeditor.upload', ['_token' => csrf_token() ])}}"
});
<?php if(isset($data['comment'])) : ?>
CKEDITOR.instances.contactEditor.setData(`<?php echo $data['comment'];?>`);
<?php endif;?>
</script>
@endif
@section("hearingLogs")
